document.addEventListener('DOMContentLoaded', function () {
    ...
    // ===== Enlaces de galería de historias =====
    const gallerySlots = Array.from(document.querySelectorAll('.home-pdf-gallery-slot, .home-pdf-gallery-slot-link'));
    ...
    if (sendWhatsAppBtn && contactWhatsAppForm) {
        sendWhatsAppBtn.addEventListener('click', function () {
            ...
            if (!target) { alert('No hay un número de WhatsApp configurado.'); return; }
            const lines = [
                `Hola, soy ${name}.`,
                email ? `Correo: ${email}` : '',
    ...
    if (sendOptionsModal) {
        sendOptionsModal.addEventListener('click', function (e) {
            if (e.target === sendOptionsModal) { closeSendModal(); }
        });
    }
